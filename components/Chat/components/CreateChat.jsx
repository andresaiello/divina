...
export default class CreateChat extends React.PureComponent {
  ...
  handleEditMembers = (member) => {
    ...
    const newSelectedMembers = (exist) ? selectedMembers.filter(elem => elem !== member) : [...selectedMembers, member];
        ...
        {({ data, loading, error }) => {
          ...
          return (
            <StyledCreateChat>
              ...
              {stepNumber === 0 && (
              <CreateChatMemberList elements={data.profile.user.followers} selectedMembers={selectedMembers} handleEditMembers={this.handleEditMembers} />
              )}
              {stepNumber === 1 && (
              <InputChatDescription selectedMembers={selectedMembers} />
              )}
            </StyledCreateChat>
          );
        }}
